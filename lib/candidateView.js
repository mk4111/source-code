        ...
        if (!request.auth.isAuthenticated) { return reply.redirect('/login'); }
          ...
          es.get({
            ...
            response._source.firstName = response._source.fullname.split(' ')[0];
              ...
              if(response._source.contacts) {
                response._source.email = response._source.contacts.email;
                response._source.phone = response._source.contacts.phone;
              }
                            ...
                            Stage.find( {candidateId: request.params.id.toString()} ).then(candidateStages => {
                              ...
                              es.count({
                                ...
                                Promise.all(promises).then(() => {
                                 ...
                                 Candidate.findOne({ indexId: request.params.id.toString() }).populate('calls')
                                 .then(c => {
