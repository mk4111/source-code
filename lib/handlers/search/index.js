...
module.exports = function (request, reply) {
  ...
  if (request.query.req_cv) {
    filter.bool.must.push({ exists : { field : "cvDocumentLink" } });
  }
  if (request.query.req_email) {
    filter.bool.must.push({
        or: [
          { exists : { field : "contacts.email" } },
          { exists : { field : "contacts.edited_email" } }
        ]
    });
  }
  if (request.query.req_phone) {
    filter.bool.must.push({ exists : { field : "contacts.phone" } });
  }
  if (request.query.req_salary) {
    filter.bool.must.push({ exists : { field : "info.sexpected" } });
  }
  // build query
  ...
  if (request.query.q) {
    ...
    query = { 
      multi_match: {
        ...
        type: "cross_fields",
        fields: ["headline", "fullname", "location", "current", "skills.skill", "contacts.edited_email", "contacts.email"]
      }
    };
  }
