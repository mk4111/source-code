...
module.exports = function (request, reply) {
  ...
  var candidatesData = candidatesEmails.map( (entry, index) => { return { email: "alexander@global-m.co.uk", id: candidatesIDs[index], firstName: candidatesFirstNames[index], fullName: candidatesFullNames[index] }; });
        ...
        if (candidatesData.length == 1) { 
          EmailService.send(request.auth.credentials, candidatesData[0], email, client, job, countries)
            .then(() => {
              ...
              return reply.redirect("/"); // @TODO should not happen
            }).catch(e => {
              return reply("ERROR").code(302); // @TODO use boom
            });
        } else {
          EmailsJob.create({
            queueId: "Emails Queue",
            userId: request.auth.credentials.user.idGoogle,
            subject: email.subject,
            schedulled_emails: candidatesData.length
          }).then(emails_job => {
            EmailsQueue.add({
              ...
              job:job,
              countries:countries,
              emails_job_id: emails_job._id
            }).then(job => {
              emails_job.jobId = job.jobId;
              emails_job.save()
                .then(emails_job => {
                return reply.redirect('/emails_jobs/' + emails_job._id + '?back=' + request.payload.pathUrl);
              }).catch(e => {
                console.log(e);
              });
            });
