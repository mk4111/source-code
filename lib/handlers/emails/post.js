    ...
    gmail.users.messages.send(paramsEmail,
       function (errorEmail, responseEmail) {
        ...
        if (responseEmail && responseEmail.labelIds && responseEmail.labelIds.indexOf('SENT') !== -1) {
          ...
          // here replace the variables from template
...
module.exports = function (request, reply) {
  ...
  var candidatesData = candidatesEmails.map( (entry, index) => { return { email: entry, id: candidatesIDs[index], firstName: candidatesFirstNames[index] }; });
