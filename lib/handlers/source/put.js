    ...
    List.findById(request.payload.id).populate('candidates')
      .then( (list) => {
        ...
        if (request.payload.edit) {
          if (request.payload._folder && request.payload._folder.trim() && request.payload._folder != list._folder) {
            promises.push(
              Folder.findByIdAndUpdate(list._folder, { $pull: { lists: list._id } })
            );
            promises.push(
              Folder.findByIdAndUpdate(request.payload._folder, { $push: { lists: list._id } })
            );
          } 
          list.name = request.payload.name.trim();
          list.description = request.payload.description.trim();
          list._folder = request.payload._folder;
        }
