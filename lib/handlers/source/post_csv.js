    ...
    var p = new Promise(function(resolve, reject) {
      ...
      es.search({
        ...
        body: {
          query: {
            terms: {
              "contacts.emailRaw": csv_candidates.map(c => { return c.Email; })
            }
          },
        }
    ...
    p.then( candidates => {
      ...
      p.then(() => {
          ...
          Promise.all(promises).then(list_candidates => {
            List.create(Underscore.extend(request.payload, { userId:request.auth.credentials.user.idGoogle, candidates: list_candidates, origin: 'csv' }))
              .then(list => {
