...
module.exports = function (request, reply) {
  ...
  if ( mustClause.length > 0 ) {
    ...
    listSearchedCandidates(mustClause, pageNum, function (resCandidates) {
      ...
      // @TODO: this should come stright from the reuest params, doesn't make any sense 
      // to hardcode the whole thing.
      // The same doesn't make sense to keep query and the whole candidates list separated
      // this code is too ugly

      var queryString  = '?' + Querystring.stringify(request.url.query);
      if (pageNum > 1) { page_url_prev = '/query/' + (pageNum - 1) + queryString; }
      if (pageNum < page_url_next) { page_url_next = '/query/' + (pageNum + 1) + queryString }
