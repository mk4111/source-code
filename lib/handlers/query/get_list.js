  ...
  if ( mustClause.length > 0 ) {
    ...
    listSearchedCandidates(mustClause, pageNum, function (resCandidates) {
      ...
      resCandidates.candidates.forEach(function (profile) {
        ...
        var listSkillsFlattened = (() => { let r = []; Underscore.each(contact.listSkills, (s) => { r.push(s.skill); s.matched = true; }); return r; })();
        ...
        Underscore.each(allSkillSorted, (s) => {
          ...
          if (Underscore.contains(listSkillsFlattened, s.skill)) { return false; } // continue - try the next one
          listSkillsFlattened.push(s.skill);
