    ...
    listGmUsers( (errorUsers, users ) => {
      ...
      listStages(function (errStages, stages) {
        ...
        if (userConnected.admin && !request.params.user) {
          ...
          Stage.find({userId:idConnectedUser}).then(userStages => {
            ...
            // @TODO: 
            // if the promise is rejected the page will never resolve
            // wrap the promises to get as much data as possible and resolve them all 
            // even if any of them fail

            Underscore.each(clientsMap, (v,k) => {
              promises.push(
                Es.get({ index: process.env.ES_INDEX, type: process.env.ES_TYPE_GM_CLIENTS, id: k }).then(r => {
            ...
            Underscore.each(jobsMap, (v,k) => {
              promises.push(
                Es.get({ index: process.env.ES_INDEX, type: process.env.ES_TYPE_GM_JOBS, id: k }).then(r => {
                  jobsMap[k] = r._source;
                })
              );
            });
            ...
            Underscore.each(candidatesMap, (v,k) => {
              promises.push(
                Es.get({ index: process.env.ES_INDEX, type: process.env.ES_TYPE, id: k }).then(r => {
